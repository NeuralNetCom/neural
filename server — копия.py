...
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///' + os.path.join(basedir, 'neural.db')
...
class User(db.Model):
    ...
    def get_reputation(self):
        # Исправлен запрос репутации (join с User не обязателен, если мы фильтруем по Post.user_id)
        # Но чтобы работало с текущей схемой, упростим:
        # Считаем лайки на постах этого юзера
        total = 0
...
class Track(db.Model):
    ...
    genre = db.Column(db.String(50), default="Lo-Fi")
...
def seed_music_db():
    # Проверка, есть ли уже треки
    if Track.query.first(): return
    
    tracks = [
        {
            "title": "Cyber City", "artist": "Neural Beats", "genre": "Synthwave",
            "url": "https://www.soundhelix.com/examples/mp3/SoundHelix-Song-1.mp3",
            "cover": "https://images.unsplash.com/photo-1535905557558-afc4877a26fc?w=400&h=400&fit=crop"
        },
        {
            "title": "Night Drive", "artist": "LoFi Chill", "genre": "Lo-Fi",
            "url": "https://www.soundhelix.com/examples/mp3/SoundHelix-Song-8.mp3",
            "cover": "https://images.unsplash.com/photo-1614726365723-49cfae97b5ae?w=400&h=400&fit=crop"
        },
        {
            "title": "Neon Rain", "artist": "Synthwave Boy", "genre": "Electronic",
            "url": "https://www.soundhelix.com/examples/mp3/SoundHelix-Song-10.mp3",
            "cover": "https://images.unsplash.com/photo-1555680202-c86f0e12f086?w=400&h=400&fit=crop"
        },
        {
            "title": "Deep Focus", "artist": "Code Masters", "genre": "Ambient",
            "url": "https://www.soundhelix.com/examples/mp3/SoundHelix-Song-3.mp3",
            "cover": "https://images.unsplash.com/photo-1515555230216-82228b88ea98?w=400&h=400&fit=crop"
        }
    ]
    
    for t in tracks:
        db.session.add(Track(title=t['title'], artist=t['artist'], genre=t['genre'], url=t['url'], cover=t['cover']))
    db.session.commit()
...
def login():
    data = request.json
    user = User.query.filter_by(secret_code=data.get('code')).first()
    if not user: return jsonify({'error': 'Неверный код доступа.'}), 401
...
def get_user_profile(handle):
    ...
    friends_list = [{'name': f.name, 'handle': f.handle, 'avatar': f.avatar} for f in target_user.friends]
...
if __name__ == '__main__':
    with app.app_context():
        db.create_all()
        seed_music_db() # Заполнение базы музыкой при старте
